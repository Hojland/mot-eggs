...
class DetectMultiBackend(nn.Module):
    ...
    def model_type(p="path/to/model.pt"):
        ...
        export_formats = pd.DataFrame(x, columns=["Format", "Argument", "Suffix", "GPU"])
    ...
    def forward(self, imgs, size=640, augment=False, profile=False):
        ...
        with amp.autocast(autocast):
            ...
            y = non_max_suppression(
                y if self.dmb else y[0], self.conf, self.iou, self.classes, self.agnostic, self.multi_label, max_det=self.max_det
            )  # NMS
            for i in range(n):
